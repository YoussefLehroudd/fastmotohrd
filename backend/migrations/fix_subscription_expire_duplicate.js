...
async function up() {
  try {
    ...
    await db.query(`
      ...
      BEGIN
        ...
        SET @expired_subscriptions = (
          ...
          -- Only include subscriptions that don't have an email event today
          AND NOT EXISTS (
        ...
        IF @expired_subscriptions IS NOT NULL THEN
          ...
          -- Create email events (only one per day)
    ...
    console.log('Subscription expire event with duplicate prevention created successfully');
  } catch (error) {
