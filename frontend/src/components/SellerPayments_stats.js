...
const SellerPayments = () => {
  ...
  const fetchPayments = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/seller/payments', {
  ...
  const handleValidate = async (paymentId) => {
    try {
      await axios.post(
        `http://localhost:5000/api/seller/payments/${paymentId}/validate`,
        {},
        { withCredentials: true }
      );
      fetchPayments();
  ...
  const handleReject = async (paymentId) => {
    try {
      await axios.post(
        `http://localhost:5000/api/seller/payments/${paymentId}/reject`,
        {},
        { withCredentials: true }
      );
      fetchPayments();
          ...
          <Card>
            <CardContent sx={{ textAlign: 'center' }}>
              ...
              <Typography variant="h6" color="textSecondary">
                Monthly Revenue: ${parseFloat(stats.monthlyRevenue).toFixed(2)}
              </Typography>
            </CardContent>
          </Card>
        </Grid>
        <Grid item xs={12} sm={6}>
          <Card>
            <CardContent sx={{ textAlign: 'center' }}>
              ...
              <Typography variant="h3" component="div" sx={{ mb: 1 }}>
                ${parseFloat(stats.completedPayments).toFixed(2)}
              </Typography>
              <Typography variant="h6" color="textSecondary">
                Total Revenue: ${parseFloat(stats.totalRevenue).toFixed(2)}
              </Typography>
            </CardContent>
          </Card>
        </Grid>
